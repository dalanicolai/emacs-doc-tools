...
(define-minor-mode doc-poppler-mode
  ...
  (setq-local doc-scroll-number-of-pages (doc-poppler-number-of-pages)
	      doc-scroll-internal-page-sizes (doc-poppler-page-sizes)

              doc-scroll-image-data-function #'doc-poppler-image-data))
...
(defun doc-poppler-image-data (page width &optional file)
  (setq file (or file (buffer-file-name)))
  (let ((coding-system-for-read 'binary)
	(buf "poppler"))
    (call-process "pdftocairo" nil buf nil
		  "-png"
		  "-f" (number-to-string page)
		  "-singlefile"
		  "-scale-to-x" (number-to-string width)
		  "-scale-to-y" (number-to-string -1)
		  "-cropbox"
		  file
		  "-")
...
(defun doc-poppler-page-sizes (&optional file last-page)
  ...
  (let* ((lines (mapcar #'split-string
				       ...
				       "-box"
                                       "-l" (number-to-string
                                             (or last-page doc-scroll-number-of-pages))
                                       file)))
         page-sizes)
    (let ((cropboxes (seq-filter (lambda (s)
				    (string= (nth 2 s) "CropBox:"))
				  lines)))
      (mapcar (lambda (mb)
		(cons (string-to-number (nth 5 mb))
		      (string-to-number (nth 6 mb))))
	      cropboxes))))
    ;; (dolist (l lines)
